...
```{r stations-data, eval = TRUE,warning=FALSE, message=FALSE, error=FALSE}
# Download the data
...
id_station <- merge(x = id_station, y = stations, by.x = "USAFID", by.y = "USAF", all.x = TRUE, all.y = FALSE)
```
...
```{r}
state_avg_temp <- met[, .(avg_temp = mean(temp, na.rm = TRUE)), by = STATE]
...
summary_table <- met[state_avg_temp, on = "STATE", nomatch = 0][, .(
  num_entries = .N,
